...
static void app_draw_callback(Canvas* canvas, void* ctx) {
    ...
    char buffer[64];
    snprintf(buffer, sizeof(buffer), "%3.2f \n", (double)data->angle_deg);
...
static void delay_pos_start_vref(Data data, FuriHalAdcHandle* adc_handle) {
    ...
    while(on_neg_confirms <= confirm_amount && on_pos_confirms <= confirm_amount) {
        VREF = data.items[0].converter(
            adc_handle, furi_hal_adc_read(adc_handle, data.items[0].pin->channel));
        if(VREF >= eps) {
            on_pos_confirms++;
            on_neg_confirms = 0;
        } else {
...
int32_t resolver_main(void* p) {
    ...
    for(size_t i = 0; i < gpio_pins_count; i++) {
        if(gpio_pins[i].channel == FuriHalAdcChannel1 ||
           gpio_pins[i].channel == FuriHalAdcChannel2 ||
           gpio_pins[i].channel == FuriHalAdcChannel4) {
            data.count++;
        }
    }
